  ...
  useEffect(() => {
    if (match.params.id && activity.id.length === 0) {
      loadActivity(match.params.id)
        .then(() => initialFromState && setActivity(initialFromState));
    }

    return (() => clearActivity())
  }, [loadActivity, clearActivity, match.params.id, initialFromState, activity.id.length])
  ...
  return (
    <Segment clearing>
      <Form onSubmit={handleSubmitForm}>
        ...
        <Button floated="right" type="text" content="Cancel" onClick={() => history.push(`/activities`)} />
      </Form>
    </Segment>
  )
}
